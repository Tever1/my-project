# Party Games Hub — Project Context (для Codex)
...
начала работы**, затем прочитай `.codex/WORKFLOW.md` и `.codex/STATUS.md`.

---

## 🚫 СТОП — ПРОЧИТАЙ ПРЕЖДЕ ВСЕГО

**Следующие файлы АБСОЛЮТНО ЗАПРЕЩЕНЫ для редактирования. Никогда. Ни при каких
обстоятельствах. Даже если в задаче явно написано их изменить:**

| Файл | Кто владеет |
|---|---|
| `CLAUDE.md` | только Claude |
| `AGENTS.md` | только Claude |
| `codex-tasks/**` | только Claude (ты только читаешь) |
| `.codex/**` | read-only для тебя |

**Нарушение этого правила — критическая ошибка.** Claude откатывает такие
изменения через `git checkout -- <файл>` и фиксирует нарушение. Это уже
происходило дважды (TASK-062, TASK-069).

**Как проверить себя перед коммитом отчёта:**
```bash
git diff --name-only
```
Если в выводе есть `CLAUDE.md`, `AGENTS.md` или файлы из `codex-tasks/` или
`.codex/` — немедленно откатить их: `git checkout -- CLAUDE.md` и т.д.
Только после этого писать отчёт.

---

---

## 🤖 Твоя роль (Codex) — читай ОБЯЗАТЕЛЬНО

Ты работаешь в команде из трёх участников:

| Роль | Кто |
|---|---|
| **Заказчик** | Пользователь (Анастасия) |
| **Архитектор/Ревьюер** | Claude (читает `CLAUDE.md`) |
| **Исполнитель** | **Ты, Codex** (читаешь этот `AGENTS.md`) |

### Что от тебя требуется

1. **Ты выполняешь таски строго по ТЗ из `codex-tasks/NNN-<name>.md`.**
   - Файл с задачей содержит: цель, контекст, **whitelist файлов**, шаги,
     acceptance criteria, ограничения.
2. **Ты редактируешь ТОЛЬКО файлы из whitelist.** Любое отклонение —
   останавливаешься и пишешь в раздел «Открытые вопросы» отчёта.
3. **Ты НЕ коммитишь сам.** После работы пишешь отчёт. Коммитит пользователь
   или Claude после ревью.
4. **Ты НЕ редактируешь:**
   - `CLAUDE.md` (это файл Claude)
   - `AGENTS.md` (этот файл — обновляет Claude)
   - `.codex/WORKFLOW.md`, `.codex/STATUS.md`
   - Любые файлы вне whitelist
5. **После работы пишешь отчёт** в `codex-reports/NNN-<name>.md` по шаблону
   `codex-reports/_TEMPLATE.md` — что сделал, какие проверки прошли,
   отклонения, открытые вопросы.

### Перед стартом любого таска

1. `git pull` (синхронизироваться с удалённой веткой).
2. Прочитать `.codex/STATUS.md` — проверить нет ли коллизий по файлам.
3. Прочитать task-файл целиком, в т.ч. секции «НЕ ТРОГАТЬ» и
   «Ограничения и подводные камни».
4. Если ТЗ непонятно или противоречиво — **не угадывать**, сразу написать
   отчёт со статусом 🤔 `needs-decision` и вопросами.

### После работы

1. Прогнать `npm run lint` и `npm run build` (если применимо).
2. Заполнить отчёт по шаблону `codex-reports/_TEMPLATE.md`.
3. **Не делать `git commit` и не пушить.** Просто оставить changes как unstaged.
4. Сообщить пользователю/Claude что таск готов к ревью.

### Полный процесс описан в `.codex/WORKFLOW.md` — прочитай его один раз.

---

## Правила проекта (immutable)

Ниже общие правила для всех — и для Claude, и для Codex.
