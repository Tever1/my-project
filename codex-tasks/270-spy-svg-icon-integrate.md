# TASK-270 — Шпион: общий SVG-SpyIcon + замена PNG-иконок в игре

## Цель

Заменить PNG-рендер `SpyIcon` в моб-Шпионе на одобренные плоские line-SVG
иконки (из превью `/design-tokens`). Маска (`mask`) — через CSS-маску из
`public/icons/spy/mask-face.png`. Цвет иконок — `currentColor` (наследуется от
текста контекста; финальная настройка цветов карточек — в TASK-271).

PNG-иконки Шпиона НЕ удалять.

## Whitelist файлов

- `src/components/games/SpyIcon.tsx` — **создать** (общий SVG-компонент)
- `src/app/game/[roomId]/spy/page.tsx` — убрать локальный `SpyIcon`, импортировать общий; header icon
- `codex-reports/270-spy-svg-icon-integrate.md` — **отчёт (писать СЮДА разрешено)**

**НЕ трогать:** `public/icons/**`, TV-файл, design-tokens, другие игры, server.mts,
package.json, GlassCard.

---

## Шаг 1 — `src/components/games/SpyIcon.tsx`

Inline-SVG набор (13 иконок, viewBox 24) + `mask` через CSS-маску. Цвет —
`currentColor` (НЕ форсим дефолт; иконка наследует цвет родителя). Дефолтный
`className` сохраняем как у старого локального компонента, чтобы инлайновые
вызовы не сломались.
...
## Шаг 2 — `src/app/game/[roomId]/spy/page.tsx`

1. Удалить локальную функцию `SpyIcon` (≈ строки 228–231, `<img>`-версия).
2. Добавить импорт: `import { SpyIcon } from '@/components/games/SpyIcon';`
   (использовать тот же алиас, что у других импортов в файле; рядом импортится
   `GameLayout` из `@/components/games/...`).
3. Header GameLayout (строка 932): `icon="/icons/spy/mask.png"` →
   `icon={<SpyIcon name="mask" className="h-7 w-7 text-teal-300" />}`.
   (GameLayout уже принимает `string | ReactNode` — менять GameLayout НЕ нужно.)
4. Остальные `<SpyIcon name="..." className="..." />` остаются по API — теперь
   резолвятся в SVG/маску, цвет наследуется от текста (currentColor).

## Acceptance

- `npx tsc --noEmit` / `npm run lint` — без новых ошибок.
- grep: в `spy/page.tsx` нет `/icons/spy/` и нет локальной `function SpyIcon`.
- Все имена, что использует Шпион (`mask, speech, palette, ballot, check, cross,
  hide, refresh, shield, trophy`), есть в `SpyIconName`.
- PNG Шпиона не тронуты. GlassCard/TV/design-tokens не тронуты.
- НЕ коммитить. Отчёт → `codex-reports/270-spy-svg-icon-integrate.md`.
