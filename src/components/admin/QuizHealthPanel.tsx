...
import { quizQualityIssueIds, type QuizQualityIssues } from '@/lib/content/quality-report';

export function QuizHealthPanel({ questions, published, issues, onAnalyze, onFilter }: { questions: ContentQuestion[]; published: ContentQuestion[]; issues: QuizQualityIssues | null; onAnalyze: () => void; onFilter: (ids: string[] | null) => void }) {
  const publishedById = new Map(published.map(question => [question.id, question]));
  const issueIds = issues ? quizQualityIssueIds(issues, questions) : [];
  ...
  return <section className="mb-5 rounded-2xl border border-white/10 bg-white/[0.035] p-4" aria-label="Здоровье квиза">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-sm font-semibold">Здоровье квиза</h3><p className="mt-1 text-xs text-slate-400">Локальные проверки не используют модель и не расходуют лимиты. Расчёт запускается только по кнопке.</p></div>
      <div className="flex flex-wrap items-center gap-2">
        <button className="rounded-xl border border-white/15 px-3 py-2 text-xs hover:bg-white/10" onClick={onAnalyze}>Проверить качество</button>
        <button className="rounded-xl border border-white/15 px-3 py-2 text-xs hover:bg-white/10 disabled:opacity-40" disabled={!issues} onClick={() => onFilter(issueIds.length ? issueIds : null)}>Показать замечания ({issueIds.length})</button>
      </div></div>
    <p className="mt-3 text-[11px] text-slate-400">{issues ? `Замечания рассчитаны: ${issueIds.length} из ${questions.length}.` : 'Замечания ещё не рассчитаны — нажмите «Проверить качество».'}</p>
