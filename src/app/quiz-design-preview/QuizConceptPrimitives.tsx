...
function DeviceLabel({ tv = false }: { tv?: boolean }) {
  return (
    <div className="mb-3 flex items-end justify-between font-mono text-[10px] uppercase tracking-[0.18em] text-white/40">
      <b className="text-white/70">{tv ? 'Общий экран' : 'Телефон игрока'}</b>
...
export function SpecialBackdrop({ special }: { special: boolean }) {
  if (!special) return null;
  return (
    <div className="absolute inset-0">
      <div
        className="absolute inset-0 bg-cover bg-center"
        style={{ backgroundImage: "url('/backgrounds/harry-potter.webp')" }}
      />
      <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(5,8,16,.34),rgba(5,8,16,.18)_40%,rgba(5,8,16,.48))]" />
    </div>
  );
}
