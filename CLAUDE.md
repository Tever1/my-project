...
6. ⬜ **Оставшиеся баги из QA** — приоритеты ниже (см. реестр ниже).
...
| Крокодил | 6/10 | ⚠️ Нужны правки (P1+P2) |
| 100 к 1 | ?/10 | ⚠️ Не доtestировано (застряли на team names) |
| Мафия | 2/10 | 🔴 NOT SHIPPABLE — 4×P0 |

### Полный реестр багов

| # | Игра | Баг | Серьёзность |
|---|------|-----|-------------|
| 1 | Крокодил | Счётчик "Раунд 5/4" после конца игры (off-by-one) | P2 |
| 2 | Крокодил | Пустой экран у не-хостов после финала | P1 |
| 3 | Шпион TV | Raw player ID вместо имени хоста в таблице ходов | P2 |
...
| 11 | Мафия | Отключившийся игрок показывается как raw `user_xxxx` на экране итогов | P2 |
| 12 | Кто я? TV | TV показывает "Game in progress" даже после окончания игры | P2 |

### Мафия — root cause P0 багов
Все role-action хэндлеры хранят состояние **только локально** (`setGs`), а не на сервере.
Сервер просто бродкастит `game:action`, но клиенты фильтруют: `if (action !== 'mafia') return` —
из-за этого `mafia-vote`, `detective-check`, `mafia-day-vote` дропаются у всех клиентов.
**Фикс:** серверная агрегация голосов + правильный роутинг action-кодов.
