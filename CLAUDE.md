...
### Спецификация: расширение ролей Мафии (зафиксирована 2026-07-27, НЕ РЕАЛИЗОВАНА)

Справочная таблица ролей в `/admin` уже расширена до 7 ролей (TASK-379,
некоммичено на момент записи). Реальная игровая механика (`assignRoles()`,
ночные фазы, типы `MafiaRole`) — по-прежнему только 4 роли (mafia/detective/
doctor/citizen). Ниже — согласованная с пользователем спецификация новых
ролей, ждёт отдельного продуктового решения "приступать к реализации" и
отдельного complex-цикла ТЗ для Codex (assignRoles, ночные фазы, типы,
вероятно server-side в socket-handlers.mts).

**Пороги ввода** (уже в таблице `/admin`): Шериф — с 4 игроков, Доктор — с 7,
Дон и Маньяк — с 10, Любовница — с 12.

**Ночной порядок разрешения** (важно для реализации — блокировки должны
применяться раньше заблокированных действий):

1. **Мафия + Дон** — просыпаются вместе **каждую ночь** (не только первую),
   выбирают жертву; при разногласиях решающий голос за Доном. В первую ночь
   дополнительно «знакомятся».
2. **Любовница** — выбирает игрока, блокирует его способность этой ночью:
   доктор не лечит, шериф не проверяет, маньяк не убивает, Дон не проверяет
   на шерифа (см. п.6). **Мафию/её выстрел не блокирует** — мафия уже
   отстрелялась на шаге 1, вне досягаемости Любовницы. Не может выбрать
   одного и того же игрока две ночи подряд. Даёт цели алиби на день: если
   днём большинство проголосует за игрока, к которому Любовница ходила этой
   ночью, — этот игрок не выбывает и остаётся в игре, несмотря на голоса.
3. **Маньяк** — убивает любого (мафию или мирного) независимо от мафии. Не
   обязан убивать каждую ночь, может не убивать вообще всю игру. Если
   заблокирован Любовницей — пропускает ход.
4. **Доктор** — лечит одного игрока (не себя, не того же две ночи подряд) от
   смерти по вине мафии **или** маньяка. Один игрок за ночь. Если
   заблокирован — пропускает ход.
5. **Шериф** — проверяет одного игрока (мафия/мирный). Если заблокирован —
   пропускает ход.
6. **Дон отдельно, последним** — проверяет одного игрока на принадлежность к
   Шерифу (ведущий подтверждает да/нет). Может проверять даже уже выбывших
   игроков. Не может напрямую сообщить находку группировке — только
   намекать днём на голосовании.

**Особый случай:** если Любовницу убивают ночью — игрок, к которому она в эту
ночь ходила, погибает вместе с ней.

**Термин «тюрьма»** — не роль, а обозначение исхода дневного голосования: за
кого проголосовало большинство, тот выбывает и открывает карту. Алиби от
Любовницы отменяет это для её цели.

**Уточнения по итогам TASK-381 (реализация в коде, 2026-07-27):**

- **Победа Маньяка (соло)** — побеждает, когда остаётся единственным живым
  игроком в игре (`alive.length === 1` и это Маньяк).
- **Голос Дона за жертву мафии** — решающий **только при разногласии/
  равенстве голосов** внутри мафии. Если у мафии уже есть чистое
  большинство без Дона — идёт по большинству, а не всегда по голосу Дона.
- **Ничья на дневном голосовании (казнь) — протокол переголосования:**
  1. Если по итогам голосования у нескольких кандидатов поровну голосов —
     эти кандидаты высказываются повторно, голосование по ним повторяется
     (только среди них, город голосует заново).
  2. Если снова ничья — выносится единое голосование: казнить ОБОИХ или
     помиловать ОБОИХ (бинарный выбор для всего города).
  3. Если и это голосование даёт ничью — оба подсудимых оправданы, никто
     не выбывает, наступает ночь.
